...
4. **Basic programming** (10 points)
    ...
    ```{r}
        x <- 5 #  Could also read from keyboard: x <- as.numeric(readline("enter the value for x:"))
        n <- 2 # Could also read from keyboard: as.integer(readline("enter an integer for n:"))
        h <- 0
        for (i in 0:n){
          h <- h+x**i
        }
