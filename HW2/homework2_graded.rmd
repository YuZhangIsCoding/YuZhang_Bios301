...
## Grade = 55 - 4 = 51
### Comments:
   * Please next time show your results for a type of questions like question 2. For example you could have shown the first ten rows of your data: haart.df[1:5,]
   *  When you choose names for the data frame, try not to pick numbers or names with blank spaces in them.
   *  When you refer to a variable within a data.frame it is better to refer it by a name and not by a column index: haart.df[, "followUpTime"] instead of haart.df[,14], this way you know exactly what variable you refer to.
   * Question 1: The difference between methods was not noticeable because your initial guess of the root value was too close to the actual root. If the initial guess was, say 5 or 10, the difference would have been noticeable. (Please see Cole's solution)
   * Question 2.3: The follow-up time should have been censored at 365 days. Please see Cole's solution.
   * Question 2.5: -2 points. Unfortunately, I didn't find the required variables. Please see Cole's solution.
   * Question 2.6: -2 points. This question implies that the same data-cleaning techniques were applied to the additional data. Please see Cole's solution.
